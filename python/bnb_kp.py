...
class bnb(knapsack):
    ...
    def branch_and_bound(self, final_sol):
        ...
        self.frac_bound(self.pqueue[1],0)
        current_best = self.pqueue[1].val
        ...
        # RETURN
        while(len(self.pqueue) > 1 and self.pqueue[1].bound > current_best):
            s = self.removeMax()
            child0 = s.copy()
            child0.fixed += 1
            child0.solution_vec[child0.fixed] = False

            child1 = s.copy()
            child1.fixed += 1
            child1.solution_vec[child1.fixed] = True

            child = [child1, child0]
            for c in child:
                self.frac_bound(c,c.fixed)
                if c.val > current_best:
                    current_best = c.val
                    self.copy_array(c.solution_vec, final_sol)
                if c.val > 0:
                    self.insert(c)
